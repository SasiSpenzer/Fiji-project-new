<script>
    $(function() {
        ...
        var usersTable = $('#branch_facility_table');
        var resultsContainer = $('#search_results');
        var usersCount = $('#facility_count');
        var clearSearchTrigger = $('.clear-search');
        var searchform = $('#facility_search_box');
        var searchformInput = $('#facility_search_box');
        ...
        $('#facility_search_box').keydown(function (e){
            if(e.keyCode == 13){
                e.preventDefault();
            }
        })
        ...
        searchform.submit(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: "{{ route('branch-facility-search') }}",
                ...
                success: function (result) {
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            ...
                            let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="branch-facility/' + val.id + '" data-toggle="tooltip" title="{{ trans("usersmanagement.tooltips.edit") }}">{!! trans("organization.buttons.edit") !!}</a>';
        ...
        searchformInput.keyup(function(event) {
            if ($('#branch_search_box').val() != '') {
                ...
                resultsContainer.html('');
                usersTable.show();
                cardTitle.html(" {!! trans('organization.showing-all-facilities') !!}");
                userPagination.show();
                usersCount.html(" ");
            };
        });
