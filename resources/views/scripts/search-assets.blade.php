    ...
    $(function() {
                ...
                success: function (result) {
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            ...
                            let editCellHtml = '<a class="" href="branch-type/' + val.id + '" data-toggle="tooltip" title="{{ trans("usersmanagement.tooltips.edit") }}"><span class="badge badge-primary"><h6>Edit</h6></span></a>';
                            let deleteCellHtml = '<form method="POST" action="branch-type-delete/'+ val.id +'" accept-charset="UTF-8" data-toggle="tooltip" title="Delete">' +
                                    '{!! Form::hidden("_method", "DELETE") !!}' +
                                    '{!! csrf_field() !!}' +
                                    '<span class="badge badge-danger"  data-toggle="modal" data-target="#confirmDelete" data-title="Delete Asset" data-message="{!! trans("usersmanagement.modals.delete_user_message", ["user" => "'+val.name+'"]) !!}">' +
                                        '<h6>Delete</h6>' +
                                    '</span>' +
                                '</form>';
        ...
        searchformInput.keyup(function(event) {
            if ($('#branch_search_box').val() != '') {
                ...
                resultsContainer.html('');
                usersTable.show();
                cardTitle.html("Show All Assets");
                userPagination.show();
                usersCount.html(" ");
            };
        });
